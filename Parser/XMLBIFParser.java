...
public class XMLBIFParser {
	...
	protected BayesianNetwork processDocument(Document doc) {
		final BayesianNetwork network = new Classes.BayesianNetwork();
	...
	protected void processVariableElement(Element e, BayesianNetwork network) {
		...
		final Domain range = new Classes.Domain();
		doForEachChild(e, "OUTCOME", new ElementTaker() {
			public void element(Element e) {
				...
				range.add(new Classes.StringValue(value));
			}
		});
		RandomVariable var = new Classes.NamedVariable(name, range);
		network.add(var);
	}
	...
	public void initCPTFromString(CPT cpt, List<RandomVariable> givens, String str) throws NumberFormatException, CPTFormatException {
		...
		recursivelyInitCPT(cpt, new Classes.Assignment(), givens, tokens);
	}
	 ...
	 *     <ul>
	 *       <li>Then use the given ArrayMapAssignment (of values to parents) for the
	 *           ``row'' of the CPT</li>
	 ...
	 *          assignment to the ArrayMapAssignment and recurse on the rest of the
	 *          givens.</li>
	 *     </ul>
	 *   </li>
	 * </ol>
	...
	protected void recursivelyInitCPT(CPT cpt, Assignment a, List<RandomVariable> givens, StringTokenizer tokens) {
		if (givens.isEmpty()) {
			// No givens: Set probabilities for given ArrayMapAssignment 
