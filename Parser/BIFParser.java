...
/* Line 85 of "BIFParser.y"  */
  ...
  public BayesianNetwork parseNetwork() throws IOException {
	  network = new Classes.BayesianNetwork();
	  if (parse()) {
		  return network;
	  } else {
		  return null;
  ...
  protected void defineVariable(String name, VariableContentItemList items) {
	  ...
	  if (network != null) {
		  Domain range = new Classes.Domain(items.size());
		  ...
		  RandomVariable variable = new Classes.RandomVariable(name, range);
	  ...
	  if (network != null) {
		  ...
		  for (ProbabilityContentEntry entry : entries) {
			  if (entry instanceof ProbabilityEntry) {
				  ...
				  Assignment a = new Classes.Assignment();
				  Iterator<String> values = pe.values.iterator();
				  for (RandomVariable given : givens) {
					  Value val = new Classes.StringValue(values.next());
					  a.put(given, val);
				  }
